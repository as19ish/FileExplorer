...
<body>
  	...
  	if (!uploading && fileName != '' && (checkValue($('#applocation1')) || checkValue($('#applocation1')))) {
        ...
        $.ajax({
            ...
            success: function (data) {
	                ...
	                for(var i = 0;i<2;i++){
	                	if(data[i] != null){
	                	result += "<tr><td>"+data[i]['file']+"</td><td>"+data[i]['msg']+"</td>"+"<td><a target='_blank' href='"+encodeURI(data[i]['link'])+"'><i class='fas fa-link'></i></a></td></tr>";
	                	}
	                }
  		...
  		alert("Please Enter location and drop file");
  	}
  ...
  $('#applocation1').keyup(function() {
	    $('#appendPath1').html($(this).val());
	});
  $('#applocation2').keyup(function() {
	    $('#appendPath2').html($(this).val());
	});
